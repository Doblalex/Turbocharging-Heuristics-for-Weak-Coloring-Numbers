...
void Usage()
{
    cerr << "Usage: ./DegreeHeuristic --in graph.txtg --rad radius [TURBOCHARGING] [--draw]" << endl;
...
int main(int argc, char **argv)
{
    ...
    while (!ordering.IsValidFull())
    {
        ...
        int vertex;
        for (auto v : vector<int>(full.begin(), full.end()))
        {
            if (graph[v].wreach_sz == target_k && graph[v].position == NOT_PLACED)
            {
                vertex = v;
                foundfull = true;
                break;
        ...
        if (!foundfull)
        {
            if (opt_components)
            {
                ...
                if (vcomponent != -1)
                {
                    ...
                    auto it = pq.begin();
                    while (scomponent.find(*it) == scomponent.end())
                    {
                        it++;
                    }
                    vertex = *it;
