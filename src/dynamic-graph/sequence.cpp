...
namespace sequence
{
  ...
  Element::Element(const Element &other)
      : id_{other.id_}, children_{nullptr, nullptr}, parent_{nullptr}, priority_{priority_distribution(random_generator)}, node_data_{other.node_data_}, subtree_data_{other.subtree_data_}
  {
    ASSERT_MSG_ALWAYS(
        other.parent_ == nullptr && other.children_[Direction::kLeft] == nullptr && other.children_[Direction::kRight] == nullptr,
        "Copied element cannot live in a sequence of multiple elements");
  }

  Element::Element(Element &&other) noexcept
      : id_{other.id_}, children_{other.children_}, parent_{other.parent_}, priority_{other.priority_}, node_data_{other.node_data_}, subtree_data_{other.subtree_data_}
  {
    if (parent_ != nullptr)
    {
  ...
  void Element::UpdateSubtreeData()
  {
    ...
    for (std::size_t i = 0; i < subtree_data_.has_marked.size(); i++)
    {
      subtree_data_.has_marked[i] =
          node_data_.marked[i] || left_subtree_data.has_marked[i] || right_subtree_data.has_marked[i];
    }
  }
  ...
  Element *Element::Split()
  {
    ...
    while (successor != nullptr && successor->children_[Direction::kLeft] != nullptr)
    {
  ...
  void Element::Mark(int32_t index, bool marked)
  {
    ...
    while (current != nullptr)
    {
      ...
      current->subtree_data_.has_marked[index] =
          current->node_data_.marked[index] || left_subtree_has_marked || right_subtree_has_marked;
  ...
  std::optional<Element *> Element::FindMarkedElement(int32_t index) const
  {
    ...
    while (true)
    {
      ...
      current =
          (current->children_[Direction::kLeft] != nullptr && current->children_[Direction::kLeft]->subtree_data_.has_marked[index])
