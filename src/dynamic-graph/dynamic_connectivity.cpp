...
namespace
{
  ...
  {
    ASSERT_MSG(
        0 <= edge.first && edge.first < num_vertices && 0 <= edge.second && edge.second < num_vertices,
...
DynamicConnectivity::DynamicConnectivity(DynamicConnectivity &&other) noexcept
    : num_vertices_{other.num_vertices_}, spanning_forests_{std::move(other.spanning_forests_)}, non_tree_adjacency_lists_{std::move(other.non_tree_adjacency_lists_)}, edges_{std::move(other.edges_)} {}
...
void DynamicConnectivity::ReplaceTreeEdge(const UndirectedEdge &edge, Level level)
{
